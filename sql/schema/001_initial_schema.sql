...
CREATE TABLE customers (
    ...
    customer_type varchar(50) NOT NULL DEFAULT 'individual' CHECK (customer_type IN ('individual', 'business')),
...
CREATE TABLE addresses (
    ...
    type varchar(50) NOT NULL DEFAULT 'shipping' CHECK (type IN ('shipping', 'billing', 'both')),
...
CREATE TABLE customer_consents (
    id uuid DEFAULT uuid_generate_v7() PRIMARY KEY,
    customer_id uuid NOT NULL REFERENCES customers(id) ON DELETE CASCADE,
    consent_type varchar(100) NOT NULL CHECK (consent_type IN ('marketing', 'analytics', 'third_party', 'cookies')),
...
CREATE TABLE products (
    ...
    tags text[],
    status varchar(50) NOT NULL DEFAULT 'active' CHECK (status IN ('draft', 'active', 'discontinued', 'deleted')),
    created_at timestamptz DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamptz DEFAULT CURRENT_TIMESTAMP NOT NULL,
    deleted_at timestamptz,
    is_deleted boolean DEFAULT false NOT NULL,
...
CREATE TABLE product_variants (
    ...
    barcode varchar(100),
    status varchar(50) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'discontinued', 'deleted')),
    created_at timestamptz DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamptz DEFAULT CURRENT_TIMESTAMP NOT NULL,
    deleted_at timestamptz,
    is_deleted boolean DEFAULT false NOT NULL
...
CREATE TABLE product_prices (
    ...
    deleted_at timestamptz,
    is_deleted boolean DEFAULT false NOT NULL,
    CONSTRAINT valid_price_range CHECK (valid_to IS NULL OR valid_to > valid_from)
);
...
CREATE TABLE locations (
    ...
    name varchar(200) NOT NULL,
    type varchar(50) NOT NULL DEFAULT 'warehouse' CHECK (type IN ('warehouse', 'store', 'dropship')),
...
CREATE TABLE orders (
    ...
    status varchar(50) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded')),
    ...
    -- Metadata
    channel varchar(50) DEFAULT 'web' CHECK (channel IN ('web', 'mobile', 'api', 'phone', 'store')),
    notes text,
    tags text[],
    metadata jsonb DEFAULT '{}',
...
CREATE TABLE payment_methods (
    id uuid DEFAULT uuid_generate_v7() PRIMARY KEY,
    customer_id uuid NOT NULL REFERENCES customers(id) ON DELETE CASCADE,
    type varchar(50) NOT NULL CHECK (type IN ('card', 'bank', 'paypal', 'wallet')),
...
CREATE INDEX idx_addresses_default ON addresses(customer_id) WHERE is_default = true;
...
CREATE INDEX idx_prices_product_variant_id ON product_prices(product_variant_id);
CREATE INDEX idx_prices_valid ON product_prices(product_variant_id, valid_from, valid_to);
...
-- Inventory indexes
CREATE INDEX idx_inventory_variant_location ON inventory(product_variant_id, location_id);
CREATE INDEX idx_inventory_low_stock ON inventory(product_variant_id) WHERE quantity_available < reorder_point;
...
COMMENT ON FUNCTION extract_timestamp_from_uuidv7(uuid) IS 'Extracts creation timestamp from UUID v7 values';